...
int all_contents(void *pos)
{
    ...
    while(data!=NULL)
    {
        printf("\t \t \t  %d        :%s\n",(idx+1),convert(data->content));
        character = convert(data->date);
        printf("\t \t \tDate:");
...
void display(list_t *users)
{
    ...
    for(idx=users->head; idx!=NULL; idx=idx->next)
    {
        ...
        while(page!=NULL)
        {
            printf("\t \t \t%d)Content:%s\n",(i+1),convert(page->content));
            printf("\t \t \tDate:");
            character = convert(page->date);
            for(pos =0; character[pos] !='\0'; pos++)
            {
                if(pos ==2 || pos ==4)
                {
...
int Deleting_your_Account(list_t *Users,user_t *Acc)
{
    ...
    for (li = Users->head ; li != NULL ; li = li->next)
    {
        ...
        if(compare_id(li,id) && compare_name(li,name) && compare_pass(li,password))
        {
            ...
            printf("\t \t \tId:%s\n",convert(Acc->id));
            printf("\t \t \tPassword:%s\n",convert(Acc->password));
            list_deleted((void*)li,(void*)Users);
            break;
        }
    }
...
int compare_password(user_t *str,char *str2)
{
    user_t *str1=NULL;
    str1=(user_t*)str;
    char *pass =convert(str1->password);
